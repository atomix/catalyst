...
/**
 * Murmur2 hashing algorithm test.
 *
...
public class Murmur2HasherTest {
  ...
  public void testHash32ByteArrayIntInt() {
    Hasher hasher = new Murmur2Hasher(0x71b4954d);
    for (int i = 0; i < input.length; i++) {
      int hash = hasher.hash32(input[i]);
  ...
  public void testHash64ByteArrayIntInt() {
    Hasher hasher = new Murmur2Hasher(0x344d1f5c);
    for (int i = 0; i < input.length; i++) {
      long hash = hasher.hash64(input[i]);
      assertTrue(hash == results64_seed[i]);
    }
  }
